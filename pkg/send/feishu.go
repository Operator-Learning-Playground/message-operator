...
func (fc *FeishuClient) Send(obj *workqueue.QueueResource) error {
	if !SysConfig1.Feishu.Open {
		klog.Info("config no support feishu sending")
		return nil
	}
	...
	if err != nil {
		klog.Error("Error marshaling card message:", err)
		return err
	}
	...
	if err != nil {
		klog.Error("Error sending message:", err)
		return err
	}
	...
	if resp.StatusCode != http.StatusOK {
		klog.Error("Error response received. Status:", resp.Status)
		return err
	}
	return nil
}

// NewFeishuClient 创建邮件发送器
				...
				map[string]interface{}{
					...
					"actions": []interface{}{
						map[string]interface{}{
							...
							"type": "primary",
							"url":  "https://www.feishu.cn", // TODO: 这里可以修改
						},
					},
				},
			},
